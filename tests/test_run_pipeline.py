...
from src.core.run_pipeline import pipeline_result_to_dict, run_core_pipeline
...
def test_run_core_pipeline_combines_faza_2_outputs(tmp_path: Path) -> None:
    write_csv(
    ...
    result = pipeline_result_to_dict(run_core_pipeline(tmp_path, "DS0001", "L001"))

    assert result["inventory"]["problems"] == []
    assert len(result["normalized_dataset"]["tables"]) == 4
    assert result["validation"]["status"] == "VALID"
    assert len(result["selection"]["records"]) == 4


def test_run_core_pipeline_surfaces_missing_sources(tmp_path: Path) -> None:
    result = pipeline_result_to_dict(run_core_pipeline(tmp_path, "DS0001", "L001"))

    assert result["validation"]["status"] == "INVALID"
    assert result["selection"]["records"] == []
    assert result["inventory"]["problems"]
