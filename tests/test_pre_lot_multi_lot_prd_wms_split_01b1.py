...
class PrdTableMapping01B1Tests(unittest.TestCase):
    def test_public_matching_success_returns_rows_without_internal_fallback(self) -> None:
        ...
        with patch(
            "src.rules.pre_lot_multi_lot_prd_wms_split._confirmed_multi_lot_prd_order_numbers",
            side_effect=AssertionError("fallback should not run when public match succeeds"),
        ):
    ...
    def test_fallback_returns_confirmed_multi_lot_different_rows_after_public_no_match(self) -> None:
        result = make_result(
            "DS099904181",
            "129.26F1",
        ...
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].values["Numar Comanda"], "0030518_AE")
        self.assertEqual(rows[0].values["PRE_LOT"], "129.26F1, 129.26F2")

    def test_fallback_returns_empty_without_exact_wms_code_lot_confirmation(self) -> None:
        result = make_result(
            "DS099904181",
            "129.26F1",
            [make_prd_row("DS099904181", "129.26F1, 129.26F2")],
    ...
    def test_fallback_returns_empty_for_substring_lot_only(self) -> None:
        result = make_result(
            "DS099904181",
            "129.26F1",
    ...
    def test_fallback_returns_empty_for_product_code_mismatch(self) -> None:
        result = make_result(
            "DS099904181",
            "129.26F1",
...
class OrderTraceabilityMapping01B1Tests(unittest.TestCase):
    ...
    def test_fallback_returns_confirmed_multi_lot_different_rows_after_public_no_match(self) -> None:
        ...
        rows = order_matching_prd_rows(dataset, "DS099904181", "129.26F1")

        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].values["Numar Comanda"], "0030518_AE")
        self.assertEqual(rows[0].values["PRE_LOT"], "129.26F1, 129.26F2")

    def test_fallback_returns_empty_without_exact_wms_confirmation(self) -> None:
        dataset = make_dataset(
            [make_prd_row("DS099904181", "129.26F1, 129.26F2")],
            [make_wms_row("DS099904181", "129.26F11", "10")],
        )
    ...
    def test_fallback_returns_empty_for_substring_token(self) -> None:
        dataset = make_dataset(
            [make_prd_row("DS099904181", "129.26F11, 129.26F2")],
            [make_wms_row("DS099904181", "129.26F1", "10")],
        )
    ...
    def test_fallback_returns_empty_when_public_case_is_not_multi_lot_different(self) -> None:
        dataset = make_dataset(
