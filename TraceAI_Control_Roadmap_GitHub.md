...
# Prompt recomandat pentru ChatGPT/Codex în GitHub

```text
You are working on the private repository cesty00/TraceAI-Control.

Act as a senior Python software engineer and software architect.

Current goal:
Implement the first step of the roadmap: Data Quality Gate.

Constraints:
- Keep PR small.
- Do not put business logic in UI.
- Do not change DOCX layout yet unless explicitly required.
- Preserve existing behavior and tests.
- Add tests for new functionality.
- Prefer dataclasses and explicit typed models.
- Update CHECKPOINT.md only after tests/diagnostics pass.

Task:
Create src/quality/models.py and src/quality/data_quality_gate.py.
Implement DataQualityReport, DataQualityIssue, and a first source validation pass:
- required files exist;
- required columns exist;
- row counts are reported;
- invalid quantity values are detected.

Integrate only a compact summary into TraceabilityCase.sections.
Do not block report generation yet.
```
