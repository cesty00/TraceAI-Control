...
QUICK_AUDITOR_GUIDE_ITEMS = [
    ...
    "Confirmă sumarul Data Quality înainte de interpretarea raportului.",
    "Verifică AMONTE: produs finit, producție și livrări.",
    "Verifică AVAL: materii prime, ambalaje, auxiliare și loturi sursă.",
...
class AuditReportPolicy:
    ...
    def register_reason(self, value: object) -> str:
        ...
        if "livrare" in folded:
            return "Confirmă livrarea amonte"
    ...
    def select_lot_flows(self, rows: Sequence[ChecklistLotFlow]) -> list[ChecklistLotFlow]:
        return list(rows)[: self.max_visible_lot_flows]
...
def build_data_quality_summary_section(report: AuditChecklistReport, data_quality_summary: dict[str, Any] | None, policy: AuditReportPolicy) -> list[str]:
    ...
    parts = [
        ...
        literal_paragraph("Sumarul Data Quality este preluat din modelul audit existent. El nu schimbă verdictul raportului și nu înlocuiește verificarea documentelor fizice.", spacing_after=50),
        table(
            ["Indicator", "Valoare"],
            [
    ...
    if issue_rows:
        parts.extend([paragraph("Observații Data Quality", style="Heading2"), table(["Severitate", "Sursă", "Loc", "Observație"], issue_rows)])
    return parts
...
def build_title_block(report: AuditChecklistReport, build_info: BuildInfo) -> list[str]:
    return [
        ...
        literal_paragraph("Raport completat din fișierele sursă disponibile: WMS trasabilitate, raport producție, stoc la moment și nomenclator.", align="center", spacing_after=40),
    ...
    return [
        ...
        table(
            ...
            [
                ...
                ["Amonte / produs finit, producție și livrări", f"{len(report.downstream)} livrări identificate" if report.downstream else MISSING],
                ["Aval / materii prime, ambalaje și loturi sursă", f"{len(report.upstream)} linii aval identificate" if report.upstream else MISSING],
                ["Documente fizice", f"{len(report.document_register)} documente de pregătit" if report.document_register else MISSING],
            ],
        ),
        *build_quick_auditor_guide_section(),
    ]


def build_quick_auditor_guide_section() -> list[str]:
    return [
        paragraph("Ghid rapid pentru auditor", style="Heading2"),
        literal_paragraph("Ghidul rapid indică ordinea recomandată de citire: verdict, Data Quality, conformare checklist, AMONTE, AVAL, consumuri, fluxuri, registru și build.", spacing_after=50),
...
def build_amonte_overview_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    main_order = primary_production_order(report)
    return [
        paragraph("Repere produs finit și lot auditat", style="Heading2"),
        paragraph("AMONTE începe cu produsul finit și lotul analizat. Reperele de mai jos fixează identificarea cazului înainte de bilanțul PRD vs WMS și livrările asociate."),
        table(["Indicator", "Valoare"], [["Cod produs finit", report.exercise.code], ["Lot produs finit", report.exercise.lot], ["Denumire produs finit", report.exercise.product_name], ["Status verificare", report.exercise.result]]),
        paragraph("Repere rapide produs finit", style="Heading2"),
        paragraph("Reperele rapide regrupează denumirea produsului, data de producție, cantitatea produsă și stocul produsului finit, pentru o citire rapidă în ordinea PP-03."),
        table(["Indicator", "Valoare"], [["Denumire produs finit", report.exercise.product_name], ["Dată producție principală", main_order.production_date if main_order else MISSING], ["Cantitate produsă PRD", report.balance.prd_produced], ["Stoc produs finit / DSD", policy.stock(report.balance.stock_at_moment)]]),
        ...
        table(["Indicator", "Cantitate / status", "Observație"], [["Cantitate produsă PRD", report.balance.prd_produced, "Sursă PRD"], ["Intrare WMS produs finit", report.balance.wms_production_out, "Intrare produs finit WMS"], ["Cantitate livrată WMS", report.balance.wms_delivered, "Valoare semnată WMS"], ["Stoc produs finit / DSD", policy.stock(report.balance.stock_at_moment), "Dacă există în stoc"], ["Status bilanț", report.balance.status, policy.short(report.balance.observation, 90)]]),
    ]
...
def build_downstream_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    ...
    if not rows:
        rows = [[MISSING] * 8]
    return [
        paragraph("02_TABEL_I_AMONTE — Produs finit, producție și livrări", style="Heading1"),
        paragraph("În terminologia PP-03, zona AMONTE acoperă produsul finit, producția și livrările. În această secțiune sunt grupate reperele de produs finit, bilanțul PRD vs WMS și livrările către client, fără recalculare și fără schimbare de mapping."),
        *build_amonte_overview_section(report, policy),
...
def build_upstream_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    ...
    if not rows:
        rows = [[MISSING] * 14]
    return [
        paragraph("03_TABEL_II_AVAL — Materii prime, ambalaje, auxiliare și loturi sursă", style="Heading1"),
        paragraph("În terminologia PP-03, zona AVAL urmărește materiile prime, ambalajele, auxiliarele și loturile sursă. Pentru fiecare lot sunt afișate consumul, recepția, furnizorul, documentul și stocul, fără recalculare."),
...
def build_third_party_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    ...
    return [
        ...
        paragraph("Această verificare evidențiază dacă loturile de materie primă folosite în produsul auditat au avut și livrări directe către terți."),
...
def build_production_consumption_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    return [
        ...
        paragraph("Această secțiune leagă comenzile de producție de cantitatea de produs finit obținută și de materialele consumate."),
...
def build_lot_flow_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    ...
    return [
        ...
        paragraph("Fluxurile de loturi reunesc informațiile esențiale despre recepții, consumul în lotul auditat, eventualele livrări către terți și stocul rămas."),
...
def build_document_register_section(report: AuditChecklistReport, policy: AuditReportPolicy) -> list[str]:
    ...
    parts = [
        ...
        literal_paragraph("Registrul indică documentele care trebuie pregătite în dosarul de audit. Coloana Bifat permite folosirea tabelului ca listă de verificare tipărită pentru documentele fizice.", spacing_after=50),
    ]
...
def literal_table_cell(value: object, is_header: bool = False, width: int | None = None) -> str:
    ...
    cell_properties = apply_cell_layout_properties(f'{width_xml}{shading_xml}<w:tcMar><w:top{xml_attrs([("w:w", 40), ("w:type", "dxa")])}/><w:left{xml_attrs([("w:w", 40), ("w:type", "dxa")])}/><w:bottom{xml_attrs([("w:w", 40), ("w:type", "dxa")])}/><w:right{xml_attrs([("w:w", 40), ("w:type", "dxa")])}/></w:tcMar>')
